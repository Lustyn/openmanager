...
export interface SessionHook {
  name: string;
  execute(context: SessionContext, options?: Record<string, unknown>): Promise<HookResult>;
}
...
export class SessionHookManager {
  ...
  async executeHook(name: string, context: SessionContext, options: Record<string, unknown> = {}): Promise<HookResult> {
  ...
  async executeHooks(names: string[], context: SessionContext, options: Record<string, unknown> = {}): Promise<HookResult[]> {
...
class CreatePatchHook implements SessionHook {
  ...
  async execute(context: SessionContext, options: Record<string, unknown> = {}): Promise<HookResult> {
    const patchOptions = options as PatchOptions;
    const worktreePath = context.worktreePath;
    const outputDir = patchOptions.outputDir ?? context.sessionRoot;
    const patchFileName = patchOptions.patchFileName ?? `session-${context.sessionId}.patch`;

    const diffResult = await execa('git', ['diff', '--binary', 'HEAD'], { cwd: worktreePath });
    ...
    if (patchOptions.includeBaseDiff) {
      const baseDiffResult = await execa('git', ['diff', '--binary', `${context.gitRef}`], { cwd: worktreePath });
      const baseDiff = baseDiffResult.stdout.trim();
      if (baseDiff) {
        const baseName = basename(patchFileName, '.patch');
        basePatchPath = resolve(outputDir, `${baseName}-vs-${sanitizeRef(context.gitRef)}.patch`);
...
class CreatePrHook implements SessionHook {
  ...
  async execute(context: SessionContext, options: Record<string, unknown> = {}): Promise<HookResult> {
    ...
    const commitTitle = prOptions.title ?? `OpenManager session ${context.sessionId}`;
    const commitBody = prOptions.description ?? `Automated changes generated by OpenManager session ${context.sessionId}.`;

    await execa('git', ['commit', '-m', commitTitle, '-m', commitBody], { cwd: worktreePath });
    ...
    return {
      success: true,
      message: autoPush ? `Branch '${branchName}' created and pushed.` : `Branch '${branchName}' created locally. Push to remote to open a PR.`,
      data: {
        branchName,
        pushed: autoPush
      }
